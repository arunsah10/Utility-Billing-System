...
const Notice = () => {
  ...
  const getNoticeHandler = () => {
    let data = {};
    if (router.pathname.replace("/", "") === "user") {
      data = {
        type: ["user", "both"],
      };
    } else {
      data = {
        type: ["user", "both", "employee"],
      };
    }
    const headers = {
  ...
  const openHandler = () => {
    setOpen(!open);
    setEdit(false);
    setData({ title: "", description: "", type: "user", link: "" });
  };
  ...
  return (
    <div className="w-full mx-auto flex justify-center items-start flex-col my-10">
      <div className="relative flex justify-between items-center w-full">
        ...
        {(router.pathname === "/employee" || router.pathname === "/admin") &&
          (open ? (
            <button
            ...
            <button
              className="absolute right-2 flex justify-center items-center border-2 border-green-500 px-3 py-2 rounded text-grey-500"
              onClick={openHandler}
            >
              Add Notice
              <span className="ml-2">
                <IoAddOutline className="text-grey-500 text-xl" />
              </span>
            </button>
          ))}
      </div>
